...
function YtCover({ videoId, title }: { videoId: string; title: string }) {
  ...
  return <img src={src} alt={`Émission ${title}`} loading="lazy" onError={() => setSrc(ytThumb(videoId, "hqdefault"))} className="h-full w-full object-cover transition-transform duration-700 group-hover:scale-105" />;
}

function ShowCover({ show }: { show: (typeof SHOWS)[number] }) {
  if ("youtubeId" in show && show.youtubeId) return <YtCover videoId={show.youtubeId} title={show.title} />;
  if ("image" in show && show.image) return <img src={show.image as string} alt={`Émission ${show.title}`} loading="lazy" className="h-full w-full object-cover transition-transform duration-700 group-hover:scale-105" />;
  return <div className="h-full w-full bg-[var(--bg-card)]" aria-hidden="true" />;
}
  ...
  return (
    <RevealSection id="shows" className="relative py-20 md:py-28" style={{ background: "var(--bg-secondary)" }}>
      <div className="mx-auto max-w-7xl px-4 md:px-8">
        <SectionLabel>{t.shows.label}</SectionLabel>
        <motion.h2 variants={fadeUp} className="font-display mt-6 text-4xl font-bold text-[var(--text-primary)] md:text-6xl">
          {t.shows.heading}{" "}
          <span className="bg-[linear-gradient(135deg,var(--accent),var(--accent-2))] bg-clip-text text-transparent">{t.shows.accent}</span>
        </motion.h2>
        <motion.p variants={fadeUp} className="mt-4 max-w-2xl text-[var(--text-secondary)]">{t.shows.sub}</motion.p>
          ...
          {SHOWS.map((show) => (
            <motion.article key={show.title} variants={fadeUp}
              ...
              <div className="relative h-52 overflow-hidden">
                ...
                <span className="font-accent absolute right-3 top-3 rounded-full border border-white/20 bg-black/50 px-3 py-1 text-[10px] font-semibold uppercase tracking-[0.12em] text-white backdrop-blur-sm">{show.badge}</span>
              </div>
              <div className="space-y-4 p-6">
                ...
                <div className="rounded-xl border border-[var(--border-subtle)] bg-[var(--accent-light)] p-3">
                  <p className="font-accent mb-1 flex items-center gap-1.5 text-[10px] font-semibold uppercase tracking-[0.16em] text-[var(--accent)]">
                    <Target className="h-3 w-3" aria-hidden="true" />{t.shows.objectif}
                  </p>
                ...
                <a href={show.href} target="_blank" rel="noreferrer"
                  ...
                  <Play className="h-3.5 w-3.5 fill-current" aria-hidden="true" />{t.shows.watch}
                </a>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {(show as typeof show & { episodes: { label: string; href: string }[] }).episodes.map((ep) => (
                      <a key={ep.label} href={ep.href} target="_blank" rel="noreferrer"
                        ...
                        aria-label={ep.label}>{ep.label}</a>
                    ))}
                  </div>
                )}
              </div>
