...
export function ContactSection() {
  const { t } = useI18n();
  const [subject, setSubject]       = useState<string>(t.contact.subjects[0]);
  const [submitting, setSubmitting] = useState(false);
  const [sent, setSent]             = useState(false);

  const INFO_ITEMS = [
    { Icon: Mail,   label: t.contact.email_label,   value: "stromaeclaudio@gmail.com", href: SOCIAL_LINKS.email,
      actions: [{ Icon: Mail, text: t.contact.email_label, href: SOCIAL_LINKS.email }] },
    { Icon: Phone,  label: t.contact.phone_label,   value: "+237 6 76 45 64 32",       href: SOCIAL_LINKS.phone,
      actions: [
        { Icon: Phone,         text: t.lang === "fr" ? "Appeler"  : "Call",     href: SOCIAL_LINKS.phone    },
        { Icon: MessageCircle, text: "WhatsApp",                                 href: SOCIAL_LINKS.whatsapp },
      ] },
    { Icon: MapPin, label: t.contact.location_label, value: t.contact.location_value,  href: null as string | null,
      sub: t.contact.location_sub, actions: [] as { Icon: typeof Mail; text: string; href: string }[] },
  ];
    ...
    <RevealSection id="contact" className="relative overflow-hidden py-20 md:py-28" style={{ background: "var(--bg-primary)" }}>
      ...
      <div className="relative mx-auto max-w-7xl px-4 md:px-8">
        ...
        <motion.h2 variants={fadeUp} className="font-display mt-6 text-4xl font-bold text-[var(--text-primary)] md:text-6xl">
          {t.contact.heading}{" "}
          <span className="bg-[linear-gradient(135deg,var(--accent),var(--accent-2))] bg-clip-text text-transparent">{t.contact.accent}</span>
        </motion.h2>
        <motion.p variants={fadeUp} className="mt-4 max-w-2xl text-[var(--text-secondary)]">{t.contact.sub}</motion.p>
        <div className="mt-12 grid gap-8 lg:grid-cols-2">
          <motion.div variants={{ hidden: { opacity: 1 }, visible: { opacity: 1, transition: { staggerChildren: 0.09 } } }} className="space-y-4">
            {INFO_ITEMS.map(({ Icon, label, value, href, sub, actions }) => (
              <motion.article key={label} variants={fadeUp}
                ...
                <div className="flex items-start gap-4">
                  <span className="rounded-xl bg-[var(--accent-light)] p-3 text-[var(--accent)]"><Icon className="h-5 w-5" /></span>
                  <div className="min-w-0 flex-1">
                    <p className="font-accent text-xs font-semibold uppercase tracking-[0.16em] text-[var(--text-muted)]">{label}</p>
                    {href ? <a href={href} className="mt-1 block truncate text-base font-semibold text-[var(--text-primary)] transition-colors hover:text-[var(--accent)]">{value}</a>
                    ...
                    {actions.length > 0 && (
                      <div className="mt-3 flex flex-wrap gap-2">
                        {actions.map(({ Icon: AIcon, text, href: ah }) => (
                          <a key={text} href={ah} target={ah.startsWith("http") ? "_blank" : undefined} rel={ah.startsWith("http") ? "noreferrer" : undefined}
            ...
            <motion.div variants={fadeUp} className="flex flex-wrap gap-2">
              {SOCIALS.map((s) => (
                <a key={s.key} href={s.href} target={s.href.startsWith("http") ? "_blank" : undefined} rel={s.href.startsWith("http") ? "noreferrer" : undefined}
                  ...
                  <PlatformIcon platform={s.key} className={`h-4 w-4 ${BRAND_COLORS[s.key] ?? "text-[var(--text-secondary)]"}`} />
                </a>
              ))}
            </motion.div>
          </motion.div>
          <motion.div variants={fadeRight} className="rounded-2xl border border-[var(--border-card)] bg-[var(--bg-card)] p-6" style={{ boxShadow: "var(--shadow-card)" }}>
            <h3 className="font-display text-2xl font-bold text-[var(--text-primary)]">{t.contact.form_title}</h3>
            <p className="mt-1 text-sm text-[var(--text-secondary)]">{t.contact.form_sub}</p>
            {sent ? (
              <div className="mt-8 flex flex-col items-center gap-3 py-10 text-center">
                <CheckCircle className="h-12 w-12 text-emerald-500" />
                <p className="font-display text-xl font-bold text-[var(--text-primary)]">{t.contact.sent_title}</p>
                <p className="text-sm text-[var(--text-secondary)]">{t.contact.sent_sub}</p>
              </div>
            ) : (
              <form onSubmit={handleSubmit} className="mt-6 space-y-4">
                <input required type="text" placeholder={t.contact.name_placeholder} aria-label={t.contact.name_placeholder}
                ...
                <input required type="email" placeholder={t.contact.email_placeholder} aria-label={t.contact.email_placeholder}
                  className="w-full rounded-xl border px-4 py-3 text-sm transition-all focus:outline-none focus:ring-2 focus:ring-[var(--accent)]/40"
                  style={{ background: "var(--input-bg)", borderColor: "var(--input-border)", color: "var(--text-primary)" }} />
                <select value={subject} onChange={(e) => setSubject(e.target.value)} aria-label={t.contact.subject_label}
                  className="w-full rounded-xl border px-4 py-3 text-sm transition-all focus:outline-none focus:ring-2 focus:ring-[var(--accent)]/40"
                  style={{ background: "var(--input-bg)", borderColor: "var(--input-border)", color: "var(--text-primary)" }}>
                  {t.contact.subjects.map((o) => <option key={o} value={o}>{o}</option>)}
                </select>
                <textarea required rows={4} placeholder={t.contact.message_placeholder} aria-label={t.contact.message_placeholder}
                ...
                <motion.button type="submit" disabled={submitting} whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.97 }}
                  ...
                  {submitting ? (
                    ...
                    <><Send className="h-4 w-4" aria-hidden="true" />{t.contact.send}</>
                  )}
                </motion.button>
                <p className="flex items-center gap-1.5 text-xs text-[var(--text-muted)]">
                  <Lock className="h-3 w-3" aria-hidden="true" />{t.contact.privacy}
                </p>
              </form>
            )}
          </motion.div>
